...
class Laporan_jasa_piutang extends CI_Controller {
	public function index()
	{
		check_not_login();
		$this->load->model("Laporan_jasa_piutang_model");
	   $data["data_anggota"] = $this->Laporan_jasa_piutang_model->get_list();
	  // $data["cek01"] = $this->Laporan_jasa_piutang_model->cek01();
	   $data["tahun"] = $this->Laporan_jasa_piutang_model->tahun();
	 //  if($cek01->num_rows() )
        $data["bln_1"] = $this->Laporan_jasa_piutang_model->get_jum();
		$this->template->load('template', 'Laporan_jasa/Laporan_jasa_v',  $data);
	}
	function export() {
		$thn = $_POST['tahun'];
		$this->load->model("Laporan_jasa_piutang_model");
		$d = $_POST['tahun'];
		$data["excel"] = $this->Laporan_jasa_piutang_model->cek01($d);
		$data["nama"] = $this->Laporan_jasa_piutang_model->get_list();
		$data["saldo_s"] = $this->Laporan_jasa_piutang_model->saldo_sebelum();
		$data["jan"] = $this->Laporan_jasa_piutang_model->jan();
		$data["feb"] = $this->Laporan_jasa_piutang_model->feb();
		$data["mar"] = $this->Laporan_jasa_piutang_model->mar();
		$data["apr"] = $this->Laporan_jasa_piutang_model->apr();
		$data["mei"] = $this->Laporan_jasa_piutang_model->mei();
		$data["jun"] = $this->Laporan_jasa_piutang_model->jun();
		$data["jul"] = $this->Laporan_jasa_piutang_model->jul();
		$data["agus"] = $this->Laporan_jasa_piutang_model->agus();
		$data["sep"] = $this->Laporan_jasa_piutang_model->sep();
		$data["okt"] = $this->Laporan_jasa_piutang_model->okt();
		$data["nov"] = $this->Laporan_jasa_piutang_model->nov();
		$data["des"] = $this->Laporan_jasa_piutang_model->des();
		$data["saldo"] = $this->Laporan_jasa_piutang_model->saldo();
		...
		$object->getProperties()->setTitle("Laporan Jasa Piutang");

		$object->setActiveSheetIndex(0);

		$object->getActiveSheet()->setCellValue('G1', 'Laporan Jasa Piutang '.$thn);
	...
	//	}
		...
		$filename="Laporan Jasa Piutang'.$thn.'".'.xlsx';

		$object->getActiveSheet()->setTitle("Laporan_Jasa_Piutang");

	    header('Content-Type: application/vnd.ms-excel');
	  	header('Content-Disposition: attachment;filename="Laporan_jasa_piutang'.$thn.'.xlsx"');
