...
class Laporan_piutang extends CI_Controller {
	public function index()
	{
	   ...
	   $data["data_anggota"] = $this->Laporan_piutang_model->get_list();
	  // $data["cek01"] = $this->Laporan_piutang_model->cek01();
	   $data["tahun"] = $this->Laporan_piutang_model->tahun();
	 //  if($cek01->num_rows() )
        $data["bln_1"] = $this->Laporan_piutang_model->get_jum();
		$this->template->load('template', 'laporan_piutang/Laporan_piutang_v',  $data);
	}
	function export() {
		$thn = $_POST['tahun'];
		$this->load->model("Laporan_piutang_model");
