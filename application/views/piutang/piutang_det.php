<section class="content-header">
      <h1>
        Detail Piutang <?php echo urldecode($this->uri->segment(4)) ?>
        <small></small>
      </h1>
      <ol class="breadcrumb">
          ...
          <div class="box">
            ...
            <div class="box-body">
              ...
              <table  id="example1"   class="table table-striped table-bordered smart-form" >
                ...
                <tbody>
               <?php
                foreach ($data_piutang->result_array() as $row) {
                ?>    
                    <?php if ($row['username'] != "admin") { ?>      
                        <tr>
                            <td><?php echo $row['id_piutang'] ?> </td>
                            <?php $tanggal = $row['tanggal']; ?>
                            <td><?php echo date('d F Y', strtotime(''.$tanggal.''));?> </td>
                            <td><?php echo number_format($row['jumlah']) ?></td>
                            <td><?php echo $row['status'] ?></td>
                            <?php
                            $sd = $row['saldo_piutang'];
                                $level = $this->session->userdata('username');
                                if ($level == "admin") {                            
                            ?>
                            <td nowrap><a target="_blank" href="<?php echo site_url('piutang/invoice/'.$row['id_piutang']) ?>" class="btn btn-success btn-sm position-sticky"><i class="fa fa-print"></i> Cetak</a>
                            </td>
                                <?php } ?>
                        </tr>
                    <?php } ?>
               ...
               ?>
                ...
                <tr>
                  ...
                  <th></th>
                  <th></th>
                  <?php foreach ($data_piutang->result_array() as $row) { ?>
                    <?php $saldo = number_format($row['saldo_piutang']) ?>
                  <?php } ?>
