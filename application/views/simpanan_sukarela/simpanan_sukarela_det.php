<section class="content-header">
      <h1>
        Detail Simpanan Sukarela <?php echo urldecode($this->uri->segment(4)) ?>
        <small></small>
      </h1>
      <ol class="breadcrumb">
               ...
               <?php
                ...
                ?>    
                    <?php if ($row['username'] != "admin") { ?>      
                        <tr>
                            <td><?php echo $row['id_sukarela'] ?> </td>
                            <?php $tanggal = $row['tanggal']; ?>
                            <td><?php echo date('d F Y', strtotime(''.$tanggal.''));?> </td>
                            <td><?php echo number_format($row['jumlah']) ?></td>
                            <td><?php echo $row['status'] ?></td>
                            <?php
                            $sd = $row['saldo_sukarela'];
                                $level = $this->session->userdata('Level');
                                if ($level == "1") {                            
                            ?>
                            <td nowrap><a target="_blank" href="<?php echo site_url('simpanan_sukarela/invoice/'.$row['id_sukarela']) ?>" class="btn btn-success btn-sm position-sticky"><i class="fa fa-print"></i> Cetak</a>
                            </td>
                                <?php } ?>
                        </tr>
                    <?php } ?>
               ...
               ?>
                ...
                <tr>
                  ...
                  <?php foreach ($table_data->result_array() as $row) { ?>
                    <?php $saldo = number_format($row['saldo_sukarela']) ?>
                  <?php } ?>
