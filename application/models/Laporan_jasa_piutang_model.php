...
class Laporan_jasa_piutang_model extends CI_Model {
    
    function get_list(){
        $this->db->DISTINCT("nama");
        $this->db->where('Level', "2");
        $this->db->select("nama, Level, id_anggota")->from("anggota");
        //$this->db->join("jasa", "anggota.id_anggota = jasa.id_anggota");
        $q = $this->db->get();
    ...
    public function cek01($id = null) {
      //  $this->db->where('month(tanggal)', "05");
        ...
        //$this->db->where("id_jasa", "max(id_jasa)");
        $q = $this->db->query("select jumlah, DATE_FORMAT(tanggal, '%m-%Y'), tanggal, id_anggota from jasa where YEAR(tanggal) = '".$_POST['tahun']."' AND
		 id_jasa in (select id_jasa from jasa GROUP BY id_anggota, MONTH(tanggal))");
		$cek01 = $q->result_array();
		return $cek01;
		
    }
    public function jan() {
      $this->db->select("SUM(jumlah) as jum, id_anggota")->from("jasa");
    ...
    function saldo() {
      $sebelum = $_POST['tahun'];
      $q = $this->db->query("select saldo_jasa, DATE_FORMAT(tanggal, '%m-%Y'), tanggal, id_anggota from jasa where YEAR(tanggal) = '".$sebelum."' AND
       id_jasa in (select MAX(id_jasa) from jasa GROUP BY id_anggota)");
      $cek01 = $q->result_array();
      return $cek01;
    ...
    function saldo_sebelum() {
      ...
      $q = $this->db->query("select saldo_jasa, DATE_FORMAT(tanggal, '%m-%Y'), tanggal, id_anggota from jasa where YEAR(tanggal) = '".$sebelum."' AND
       id_jasa in (select id_jasa from jasa GROUP BY id_anggota)");
      $cek01 = $q->result_array();
      return $cek01;
