...
(function( $ ) {
  ...
  $.floatThead.defaults = {
    ...
    autoReflow: true, //(undocumented) - use MutationObserver api to reflow automatically when internal table DOM changes
      ...
      function columnNum(){
        ...
        if(count !== lastColumnCount){
          ...
          for(var x = 0; x < count; x++){
			...
			if(content){
				cells.push('<th class="floatThead-col" aria-label="'+content+'">'+content+'</th>');
			}
			cols.push('<col/>');
            psuedo.push(
              $('<fthtd>').css({
