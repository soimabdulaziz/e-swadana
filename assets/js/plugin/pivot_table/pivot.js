  ...
  callWithJQuery(function($) {
        ...
        return function(arg) {
          ...
          return function(data, rowKey, colKey) {
            return {
              ...
              format: formatter,
              numInputs: (num != null) && (denom != null) ? 0 : 4
            };
          };
        };
      },
    ...
    aggregators = (function(tpl) {
      return {
        ...
        //"Maxdate": tpl.sum(usFmt),
        //"First": tpl.first(usFmt),
        //"Last": tpl.last(usFmt),
        "Sum Compare": tpl.sumOverSum(usFmt),
    ...
    pivotTableRenderer = function(pivotData, opts) {
      ...
      for (j in colAttrs) {
		...
		var agg_name = $(".pvtAggregator").val();
		if(agg_name!="Sum Compare"){
    ...
    $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
      ...
      try {
        ...
        rendererControl = $("<td>").addClass("pvtUiCell firstTable summaryBox");
        renderer = $("<select>").addClass('pvtRenderer').appendTo(rendererControl).bind("change", function() {
		  $("#app_btn").removeClass("btn-success");
          return refresh();
        });
        ref = opts.renderers;
        for (x in ref) {
        ...
        unused = $("<td>").addClass('pvtAxisContainer pvtUnused pvtUiCell hellBox');
        ...
        fn1 = function(attr) {
		  ...
		  $("<button>", {
				type: "button"
			  }).text("x").appendTo(valueList).bind("click", function() {
          ...
          attrElem = $("<li>").addClass("axis_" + i).append($("<span>").addClass('pvtAttr').append("<i>").addClass(icon).text("  "+attr.toUpperCase()).data("attrName", attr).append(triangleLink)).attr("title",attr.toUpperCase());
          if (hasExcludedItem) {
            attrElem.addClass('pvtFilteredAttribute');
          }
		...
		fixedButton = $("<a id='fixedBtn'>", {
          role: "button"
        }).addClass("pvtColOrder").html("<i class='fa fa-table'></i>").bind("click", function() {
           fixedtable();
        });
        $("<td>").addClass('pvtVals pvtUiCell summaryBox selectBox').appendTo(tr1).append($("<div class='btn_box'></div>")).append(aggregator).append(rowOrderArrow).append(colOrderArrow).append(fixedButton).append($("<br>"));
        $("<td>").addClass('pvtAxisContainer pvtHorizList pvtCols pvtUiCell dragBox').appendTo(tr1);
        tr2 = $("<tr>").appendTo(uiTable);
        tr2.append($("<td>").addClass('pvtAxisContainer pvtRows pvtUiCell dragBox').attr("valign", "top"));
        pivotTable = $("<td>").attr("valign", "top").addClass('pvtRendererArea').appendTo(tr2);
		//pivotTable.append("<div>");
        if (opts.unusedAttrsVertical === true || unusedAttrsVerticalAutoOverride) {
          ...
          return function() {
            ...
            if (numInputsToProcess !== 0) {
              ...
              for (x = t = 0, ref5 = numInputsToProcess; 0 <= ref5 ? t < ref5 : t > ref5; x = 0 <= ref5 ? ++t : --t) {
                newDropdown = $("<select>").addClass('pvtAttrDropdown select2').append($("<option>")).bind("change", function() {
				  $("#app_btn").removeClass("btn-success");
                  return refresh();
                });
			...
			$(".btn_box").remove().insertAfter($(".selectBox select:last"));
			fixed_left = subopts.rows.length+1;
